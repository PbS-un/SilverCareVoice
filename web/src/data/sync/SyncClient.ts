 ...
 *  - WS /ws：hello 註冊裝置；收 change → apply ops → 觸發 provider.subscribe
...
export class SyncClient {
  ...
  /** 首次加入：GET /sync/bootstrap 全量拉取並 LWW apply。 */
  async bootstrap(): Promise<void> {
    const res = await fetchWithTimeout('/sync/bootstrap', {}, 10_000, this.fetchImpl);
    if (!res.ok) throw new Error(`bootstrap failed: HTTP ${res.status}`);
    const body = (await res.json()) as { entities: BootstrapEntity[]; serverTime: string };
    await this.applyOps(
      body.entities.map((e) => ({
    ...
    this.writeCursor(body.serverTime);
  }

  /** 增量：GET /sync/pull?since=cursor，apply 後推進 cursor。 */
  async pull(): Promise<void> {
    const since = this.readCursor();
    if (!since) return this.bootstrap();
    const res = await fetchWithTimeout(`/sync/pull?since=${encodeURIComponent(since)}`, {}, 10_000, this.fetchImpl);
    if (!res.ok) throw new Error(`pull failed: HTTP ${res.status}`);
    const body = (await res.json()) as { ops: WireOp[]; cursor: string; serverTime: string };
    await this.applyOps(body.ops);
    this.writeCursor(body.cursor || body.serverTime);
  }
  ...
  private connect(): void {
    ...
    ws.onopen = () => {
      try {
        ws.send(JSON.stringify({ type: 'hello', deviceId: this.deviceId }));
      } catch {
        // ignore
      }
    };
    ...
    ws.onmessage = (ev: MessageEvent) => {
      let msg: { type?: string; ops?: WireOp[] };
      try {
  ...
  /**
   * 把一批遠端 ops apply 到本地（LWW by updatedAt，與 server 同策略：
   * 平手保留本地）。寫入直接走底層 IndexedDBProvider —— 其 bulkPut /
   * remove 會 emit，從而觸發 provider.subscribe，UI 自動刷新。
   */
  async applyOps(ops: WireOp[]): Promise<void> {
  ...
  private readCursor(): string | null {
    try {
      return localStorage.getItem(LS_SYNC_CURSOR);
    } catch {
      return null;
    }
  }
